...
import { coneSurfaceArea, coneVolume } from "../../utils/cone.js";
...
export default function Cone() {
  ...
  const onSubmit = (data) => {
    const surfaceArea = coneSurfaceArea(data.RADIUS, data.HEIGHT);
    const volume = coneVolume(data.RADIUS, data.HEIGHT);
  ...
  return (
    <div className="flex flex-col lg:flex-row  gap-0 w-screen h-full justify-center mt-20 overflow-hidden">
      <div className="w-full lg:w-[30%]">
        <FormContainer formTitle={"Calculations for the Cone"} onSubmit={handleSubmit(onSubmit)}>
          <FormInputField 
            definition={"Radius"} 
      ...
      <div className="w-full lg:w-[60%] min-h-full relative overflow-hidden overflow-y-auto max-w-full" style={{ overflowX: 'hidden' }}>
        <ConeWithAxes radius={radius} height={height}/>
      </div>
    </div>
  );
}
